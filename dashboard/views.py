...
# @login_required(login_url='signin')
# def customer_profile(request):
#     return render(request, 'placeholder.html', {'message': 'Customer Profiles - To be implemented'})

# @login_required(login_url='signin')
# def eda_charts(request):
#     return render(request, 'placeholder.html', {'message': 'Data Exploration - To be implemented'})


@login_required(login_url='signin')
# def cohort_analysis(request):
#     return render(request, 'placeholder.html', {'message': 'Cohort Analysis - To be implemented'})
## 6 cohort page ## 
...
def cohort_data(request):
    ...
    Uses new_db models (Order).
    """
    try:
        logger.info("Fetching cohort analysis data for new_db...")
        ...
        if start_date and end_date:
            ...
            end_date = pd.to_datetime('2025-05-28').tz_localize('UTC')
            start_date = end_date - pd.Timedelta(days=365)
        else:
            earliest_order = Order.objects.earliest('created_at')
            if earliest_order:
                start_timestamp = pd.to_datetime(earliest_order.created_at)
        ...
        # Fetch orders from new_db
        orders = Order.objects.filter(created_at__gte=start_date, created_at__lte=end_date)
        if not orders.exists():
            logger.error("No orders found within the specified date range.")
            return JsonResponse({'error': 'No orders found within the specified date range.'}, status=400)

        # Create DataFrame with relevant fields (updated fields)
        df = pd.DataFrame(list(orders.values('created_at', 'customer_email', 'total_price')))
        df['created_at'] = pd.to_datetime(df['created_at'])

        # Since customer_email is already in the Order model, no need to fetch from Customer model
...
#     return render(request, 'placeholder.html', {'message': 'Ad Strategies - To be implemented'})
...
def generate_multiple_strategies(request):
    if request.method == 'POST':
        ...
        # Sanitize the cache key by replacing invalid characters
        import re
        sanitized_prompt = re.sub(r'[^a-zA-Z0-9_-]', '_', prompt)
        cache_key = f"strategies_{sanitized_prompt}"
        
        # Cache the response to avoid repeated API calls
        cached_response = cache.get(cache_key)
        if cached_response:
            logger.info("Returning cached strategies for prompt: %s", prompt)
            return JsonResponse({'success': True, 'strategies': cached_response})
